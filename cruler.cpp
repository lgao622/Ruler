...
void CRuler::paintEvent(QPaintEvent *)
{
    ...
    if (m_orn == Qt::Horizontal) {
        for (int i = 0; i < m_widthmm; i++) {
            paint.drawLine(QPointF(i*m_pixmm + 0.5, 0), QPointF(i*m_pixmm + 0.5, (i%10==0 ? 5 : (i%5==0 ? 4 : 3))*m_pixmm));
            if (i > 0 && i%10==0) paint.drawText(QRectF(i*m_pixmm + 0.5-19, 6*m_pixmm, 40, 20), QString::number(i/10), m_to);
        }
        paint.drawText(QRectF(0, 6*m_pixmm, 40, 20), "0cm");

        for (int i = 0; i < m_width; i+=5) {
            paint.drawLine(QPointF(i, m_pixmm * 100), QPointF(i, (30 -(i%100==0 ? 5 : (i%50==0 ? 4 : 3)))*m_pixmm));
            if (i > 0 && i%100==0) paint.drawText(QRectF(i-19, 21*m_pixmm, 40, 20), QString::number(i), m_to);
        }
        ...
        for (int i = 0; i < m_height; i++) {
            paint.drawLine(QPointF(0, i*m_pixmm + 0.5), QPointF((i%10==0 ? 5 : (i%5==0 ? 4 : 3))*m_pixmm, i*m_pixmm + 0.5));
            if (i > 0 && i%10==0) paint.drawText(QRectF(6*m_pixmm, i*m_pixmm + 0.5-9, 40, 20), QString::number(i/10), m_to);
        }
        paint.drawText(QRectF(6*m_pixmm, 0, 40, 20), "0cm");

        m_to.setAlignment(Qt::AlignRight | Qt::AlignVCenter);
        for (int i = 0; i < m_height; i+=5) {
            paint.drawLine(QPointF(m_pixmm * 100, i), QPointF((30 -(i%100==0 ? 5 : (i%50==0 ? 4 : 3)))*m_pixmm, i));
            if (i > 0 && i%100==0) paint.drawText(QRectF(15*m_pixmm, i-9, 9*m_pixmm, 20), QString::number(i), m_to);
        }
        paint.drawText(QRectF(15*m_pixmm, 0, 9*m_pixmm, 20), "0pix", m_to);
    }
